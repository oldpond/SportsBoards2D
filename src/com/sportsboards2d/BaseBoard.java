...
public abstract class BaseBoard extends Interface {
	...
	protected String REF_PATH_SMALL;
	protected String REF_PATH_LARGE;
	...
	protected TiledTextureRegion mRefTextureRegion;
	 ...
	 * onLoadEngine
	 * 
	 * Creates a new engine and initializes the camera Creates the 2D engine for
	 * the program to use
	 */
	@Override
	 ...
	 * onLoadResources
	 * 
	 * Loads texture resources for menus, players, background Everything that
	 * you're going to see
	 */
	@Override
	public void onLoadResources() {
		...
		// load menu textures
		this.mPlayerInfoFontTexture = new Texture(128, 128,
				TextureOptions.BILINEAR_PREMULTIPLYALPHA);
		this.mPlayerInfoFont = FontFactory.createFromAsset(
				this.mPlayerInfoFontTexture, this, "VeraBd.ttf", 18, true,
				Color.BLACK);
		this.mEngine.getTextureManager().loadTexture(
				this.mPlayerInfoFontTexture);
		...
		this.mRefTexture = new Texture(64, 64, TextureOptions.BILINEAR);

		if (config.largePlayers) {
			this.mRedPlayerTextureRegion = TextureRegionFactory
					.createTiledFromAsset(this.mRedPlayerTexture, this,
							"48x48RED.png", 0, 0, 1, 1);
			this.mBluePlayerTextureRegion = TextureRegionFactory
					.createTiledFromAsset(this.mBluePlayerTexture, this,
							"48x48BLUE.png", 0, 0, 1, 1);
			//this.mRefTextureRegion = TextureRegionFactory.createTiledFromAsset(
			//		this.mRefTexture, this, "referee_48.png", 0, 0, 1, 1);
			this.mRefTextureRegion = TextureRegionFactory
					.createTiledFromAsset(this.mRefTexture, this,
							REF_PATH_SMALL, 0, 0, 1, 1);
			this.mBallTextureRegion = TextureRegionFactory
					.createTiledFromAsset(this.mBallTexture, this,
							BALL_PATH_SMALL, 0, 0, 1, 1);
		} else {
			this.mRedPlayerTextureRegion = TextureRegionFactory
					.createTiledFromAsset(this.mRedPlayerTexture, this,
							"32x32RED.png", 0, 0, 1, 1);
			this.mBluePlayerTextureRegion = TextureRegionFactory
					.createTiledFromAsset(this.mBluePlayerTexture, this,
							"32x32BLUE.png", 0, 0, 1, 1);
			//this.mRefTextureRegion = TextureRegionFactory.createTiledFromAsset(
			//		this.mRefTexture, this, "referee_32.png", 0, 0, 1, 1);
			this.mRefTextureRegion = TextureRegionFactory
					.createTiledFromAsset(this.mRefTexture, this,
							REF_PATH_SMALL, 0, 0, 1, 1);
			this.mBallTextureRegion = TextureRegionFactory
		...
		this.mEngine.getTextureManager().loadTextures(this.mBluePlayerTexture,
				this.mRedPlayerTexture, this.mBallTexture, this.mRefTexture);
	}

	/*
	 * onLoadScene
	 * 
	 * Initialises the scene and loads the last loaded formation The scene is
	 * what you see.
	 */
	@Override
	public Scene onLoadScene() {
		...
		this.mMainScene.getChild(BACKGROUND_LAYER).attachChild(
		...
		this.mRef = new RefSprite(0, 0, this.mRefTextureRegion);
		if (formsList != null) {
			showFormation();
		}
		return mMainScene;
	...
	private void clearPlayers() {
		for (final PlayerSprite p : playerSprites) { // for each Player...
			mEngine.runOnUpdateThread(new Runnable() {
				@Override
				public void run() {
					// find the physics object, delete it, remove the sprite and
					// unregister the touch area.
					Body body = BaseBoard.this.mPhysicsWorld
							.getPhysicsConnectorManager().findBodyByShape(p);
					BaseBoard.this.mPhysicsWorld.destroyBody(body);
					BaseBoard.this.mMainScene.getChild(PLAYER_LAYER)
							.detachChild(p);
	...
	private void clearRef() {
		...
		mEngine.runOnUpdateThread(new Runnable() {
			@Override
			public void run() {
				BaseBoard.this.mMainScene.getChild(REF_LAYER).detachChild(mRef);
				BaseBoard.this.mMainScene.unregisterTouchArea(mRef);
			}
		});
	}
	...
	private void clearLines() {
		for (final Line line : lines) {
			mEngine.runOnUpdateThread(new Runnable() {
				@Override
				public void run() {
					BaseBoard.this.mMainScene.getChild(LINE_LAYER).detachChild(
							line);
	...
	private List<PlayerObject> captureFormation() {
		...
		for (int i = 0; i < mMainScene.getChild(PLAYER_LAYER).getChildCount(); i++) {
			if (mMainScene.getChild(PLAYER_LAYER).getChild(i) instanceof PlayerSprite) {
				pSprite = (PlayerSprite) mMainScene.getChild(PLAYER_LAYER)
						.getChild(i);
				team = pSprite.getPlayer().getpTeam();
				xPlayer = pSprite.getX();
				yPlayer = pSprite.getY();
				newPlayer = pSprite.getPlayer();
				pInfo = new PlayerInfo(newPlayer.getpID(), newPlayer.getpInfo()
						.getjNum(), newPlayer.getpInfo().getType(), newPlayer
						.getpInfo().getPName());
				newPlayer = new Player(newPlayer.getpID(), team,
						new Coordinates(xPlayer, yPlayer), pInfo);
	...
	private void showFormation() {
		...
		for (PlayerObject p : fn.getPlayers()) {
			...
			body = PhysicsFactory.createBoxBody(this.mPhysicsWorld, newPlayer,
					BodyType.DynamicBody, FIXTURE_DEF);
			this.mPhysicsWorld.registerPhysicsConnector(new PhysicsConnector(
					newPlayer, body, true, true));
		...
		if (config.playerInfoDisplayToggle == true
				&& config.playerInfoDisplayWhenMode == 1) {
		...
		this.mRef.setPosition(fn.getRef().getX(), fn.getRef().getY());
		this.mMainScene.getChild(REF_LAYER).attachChild(this.mRef);
		this.mMainScene.registerTouchArea(this.mRef);
	}

	/*
	 * createPlayer
	 * 
	 * Creates a new player sprite, along with listeners for the sprite context
	 * menu
	 */
	...
	protected PlayerSprite createPlayer(final Player p, TiledTextureRegion tex) {
		...
		final PlayerSprite newPlayer = new PlayerSprite(p, p.getX(), p.getY(),
				tex) {
			...
			public boolean onMenuItemClicked(final MenuScene pMenuScene,
				...
				case Constants.PMENU_SWAP:
					...
					Intent intent1 = new Intent(BaseBoard.this,
							SelectPlayer.class);
				...
				case Constants.PMENU_HIDE:
					...
					mEngine.runOnUpdateThread(new Runnable() {
						...
						@Override
						public void run() {
							final Body body = BaseBoard.this.mPhysicsWorld
									.getPhysicsConnectorManager()
									.findBodyByShape(selectedPlayer);
							BaseBoard.this.mPhysicsWorld.destroyBody(body);
							BaseBoard.this.mMainScene.getChild(PLAYER_LAYER)
									.detachChild(selectedPlayer);
							BaseBoard.this.playerSprites.remove(selectedPlayer);
							BaseBoard.this.mMainScene
									.unregisterTouchArea(selectedPlayer);
		...
		playerText = new ChangeableText(+10, -20, this.mPlayerInfoFont, p
				.getpInfo().getInitials(), 30);
		newPlayer.addDisplayInfo(playerText);

		playerText = new ChangeableText(-25, +10, this.mPlayerInfoFont, p
				.getpInfo().getType(), 30);
		newPlayer.addDisplayInfo(playerText);

		playerText = new ChangeableText(+10, +50, this.mPlayerInfoFont,
				String.valueOf(p.getpInfo().getjNum()), 30);
		newPlayer.addDisplayInfo(playerText);
		...
		if ((pTouchArea) instanceof RefSprite) {
			...
			case TouchEvent.ACTION_MOVE:
				...
				if (this.mRef.ismGrabbed()) {
					this.mRef.setPosition(pSceneTouchEvent.getX()
							- PLAYER_OFFSET, pSceneTouchEvent.getY()
							- PLAYER_OFFSET);
				}
				return true;
			...
			case TouchEvent.ACTION_MOVE:
				...
				if (this.mBall.ismGrabbed()) {
					this.mBall.setPosition(pSceneTouchEvent.getX()
							- PLAYER_OFFSET, pSceneTouchEvent.getY()
							- PLAYER_OFFSET);
				}
				return true;
			...
			Body body = this.mPhysicsWorld.getPhysicsConnectorManager()
					.findBodyByShape(sprite);
			...
			case TouchEvent.ACTION_DOWN:
				...
				if (config.playerInfoDisplayToggle
						&& config.playerInfoDisplayWhenMode == 0) {
			...
			case TouchEvent.ACTION_MOVE:
				...
				if (sprite.ismGrabbed()) {
					...
					diff = MathUtils.distance(sprite.getStartX(),
							sprite.getStartY(), pSceneTouchEvent.getX(),
							pSceneTouchEvent.getY());

					if (diff > 25) {

						sprite.setPosition(pSceneTouchEvent.getX(),
								pSceneTouchEvent.getY());
						body.setTransform(
						...
						path.add(new Coordinates(sprite.getX() - PLAYER_OFFSET,
								sprite.getY() - PLAYER_OFFSET));

						if (config.lineEnabled) {// if drawing lines is enabled

							float relativeX = 2 * (MathUtils.bringToBounds(0,
									sprite.getWidth(), pTouchAreaLocalX)
									/ sprite.getWidth() - 0.5f);
							float relativeY = 2 * (MathUtils.bringToBounds(0,
									sprite.getHeight(), pTouchAreaLocalY)
									/ sprite.getHeight() - 0.5f);

							angleDeg = MathUtils.radToDeg((float) Math.atan2(
									-relativeX, relativeY));

							left.setPosition(moveX, moveY, moveX - 20,
									moveY - 20);
							right.setPosition(moveX, moveY, moveX + 20,
									moveY - 20);

							left.setRotation(angleDeg);
							right.setRotation(angleDeg);

							arrowLineMain = LineFactory.createLine(
									sprite.getStartX(), sprite.getStartY(),
									moveX, moveY, 8);
							this.mMainScene.getChild(LINE_LAYER).attachChild(
									arrowLineMain);
				...
				if (sprite.ismGrabbed()) {
					...
					if (recording) {
						if (path.size() > 1) {
							...
							body.setTransform(new Vector2(Xs[0] / 32,
									Ys[0] / 32), 0);
						}
					}
	...
	public boolean onMenuItemClicked(final MenuScene pMenuScene,
		...
		case Constants.MAIN_MENU_SETTINGS:
			this.startActivityForResult(new Intent(this, SettingsViewer.class),
					4);
			this.mMainMenu.back();
			return true;
			// case Constants.MAIN_MENU_PLAYBACK:
			// this.playBackEnabled = true;
			// return super.onMenuItemClicked(pMenuScene, pMenuItem,
			// pMenuItemLocalX, pMenuItemLocalY);
	 ...
	 * onActivityResult
	 * 
	 * This is called when an activity finishes (eg. saving/loading/deleting a
	 * form)
	 */
	...
	protected void onActivityResult(int requestCode, int receiveCode,
		...
		if (requestCode == 1) {
			if (receiveCode == 1) {
				...
				if (intent == null) {
					fn = new FormationObject(formsList.get(receiveCode)
							.getfName(), new Coordinates(this.mBall.getX(),
							this.mBall.getY()), new Coordinates(this.mRef.getX(), this.mRef.getY()), playerList);
					formsList.set(receiveCode + 1, fn);
				} else {
					fn = new FormationObject(intent.getType(), new Coordinates(
							this.mBall.getX(), this.mBall.getY()), new Coordinates(this.mRef.getX(), this.mRef.getY()), playerList);
					formsList.add(fn);
				}
				StringPrinting.printAllFormation(formsList);
				XMLAccess.writeFormations(this, formsList,
		...
		// delete form activity
			...
			if (receiveCode != -1) {
				...
				if (formsList.get(receiveCode).getfName()
						.equalsIgnoreCase(DEFAULT_NAME)) {
				} else {
		...
		// create a new player
			...
			Toast.makeText(
					this,
					"In order to display different players, touch and hold a player to bring up a context menu",
			...
			for (PlayerInfo pInfo : players) {
				...
				for (PlayerObject pEntry : fEntry.getPlayers()) {
					...
					if (pID == pEntry.getpID()) {
						newPlayer = new Player(pID,
								((PlayerEntry) pEntry).getpTeam(),
								((PlayerEntry) pEntry).getCoords(), pInfo);
						matchList.add(newPlayer);
					}
				}
			}
			fn = new FormationObject(fEntry.getfName(), fEntry.getBall(), fEntry.getRef(),
					matchList);
			forms.add(fn);
		}
	 ...
	 * finish()
	 * 
	 * Called when the app is closing. Saves the current formation so it is
	 * loaded on next boot
	 */
	...
	public void finish() {
		...
		editor.putString("last loaded", formsList.get(currentFormation)
				.getfName());
		editor.commit();
		super.finish();
	}
